...
async function fetchList() {
  try {
    ...
    const { error_code, message, data } = fetchStudentResponse.data;
...
function fetchClass(e) {
  ...
  updateStudentClass(selectedValue);
  updateStdClassName(selectedText);
}
  ...
  function viewStudent(student_id) {
    navigate("/viewStudent", { student_id });
  }
...
const columns = [
...

  {
    ...
    sortable: true,
    wrap: true,
    cell: (row) => <div className="ten_font_class">{row.student_id}</div>,
  },

  {
    ...
    wrap: true,
    cell: (row) => <div className="ten_font_class">{row.persona}</div>,
  },
  ...
  {
    name: "Class Name",
    selector: (row) => row.class, // corrected
    wrap: true,
    cell: (row) => (
      <div
        className="overflow_class ten_font_class"
    ...
    cell: (row) => (
      <>
        <div className="action_buttons_end_css">
          ...
          <div
            className={`edit_campus_modal actions_modal${row.student_id}`}
            style={{
              display: "none",
              position: "absolute",
              top: "22px",
